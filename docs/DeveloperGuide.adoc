...
image::LoanIdAddCommandSequenceDiagram.png[width="800"]
...
*** Cons:
.. The model needs to keep track of all the IDs that have been used before to prevent duplicates.
.. The ID may be troublesome to type in the command-line, especially if the Loan ID is used for other commands.
...
** Cons:
.. It is more complicated to capture the state of the running value. The undo/redo mechanism may not work with a static variable.

// end::loanid[]

// tag::adminauthentication[]
=== Authentication
Before critical actions such as deleting a loan can be performed, authentication of user
is required. This ensures that only authorized users are able to perform critical actions.
This is done by requiring a password before performing a critical action.

==== Current Implementation
|===
| _The following provide a walkthrough on how the authentication feature is implemented._
|===
...
==== Design Considerations
|===
|_The following aspects are considerations that our team had made before the implementation of this feature._
|===
...
[[Implementation-Bicycle-Management]]
=== Bicycle Management
...
Each `LoanBook` has its own list of `Bike` s, representing all the bicycles that a bicycle shop owner has at their disposal. Please refer to the class diagram under the <<Design-Model,`Model`>> section of this guide for a full view of how `Bike` s are handled within LoanBook.
...
image::BicycleManagementAddLoanSequenceDiagram.png[width="800"]
...
==== Editing a bike's details: `editbike`
...
==== Viewing the list of bikes in the UI: `listbikes`
...
As mentioned in the <<Design-Model,`Model`>> section earlier in this guide, a `BikeStatus` is currently present in the `Bike` class but does not do anything.
...
image::BikeStatusAddLoanActivityDiagram.png[width="600"]

And this acitivty dragram demonstrates the same for returning bicycles:

image::BikeStatusReturnLoanActivityDiagram.png[width="600"]
...
.. Prerequisites: List all loans using the `list` command. Multiple loans in the list. A Bike with the name `Bike001` is present in the Loanbook. The last used Loan ID is smaller than 999,999,999, or the Loanbook is new.
.. Test case: `add n/David ic/T0248272F p/98765432 e/david@example.com b/Bike001 lr/3.5 t/Overnight` +
   Expected: A new loan is added to the list.
... The editable details of the loan (e.g. Name, NRIC, etc.) match the fields specified in the command.
... The Loan ID of the new loan is larger than that of all existing loans.
... The "Start Time" field is equal to the time of execution of the `add` command.
... The tags that appear are "Ongoing" and "Overnight".

. Adding a loan with a bike that does not exist

.. Prerequisites: A bike with the name `MissingBike` does not exist in the Loanbook.
.. Test case: `add n/David ic/T0248272F p/98765432 e/david@example.com b/MissingBike lr/3.5 t/Overnight` +
