...
Each `AddressBook` will have its own list of `Bike`s, representing all the bicycles that a bicycle shop owner has at their disposal. For the most part, the list of bicycles will be maintained in the same way as the list of loans.
...
* `loanHistory`: The past loans associated with this bicycle, sorted chronologically.
* `loanReserves`: The future loans (i.e. reservations) associated with this bicycle, sorted chronologically.
* Traits that characterize the bicycle, such as `brand`, `model`, `top speed`, `wheel size`, `weight`, `material` etc.

[NOTE]
Why two lists to track loans, instead of collating them into one big list? Because each list has a specific category of loans and it is more common to encounter situations that deal with only one of the lists (e.g. checking transaction history, or checking for clashes with reservations), rather than both.

Every `Loan` will have a reference to exactly one `Bike`: the bicycle that is loaned out. Conversely, every `Bike` has a reference to a list of `Loan`s, which track the bicycle's loan history and future reservations.

[NOTE]
This makes the `AddressBook`'s list of loans a little harder to manage, due to the extra references. However, this has been done anyway as referencing a bicycle's history is a very useful and frequently used operation. To manage the deletion of loans, the loan will be dereferenced from from the Bike it references before it gets deleted. The sequence diagram below demonstrates this process:
image::DeleteLoanSequenceDiagram.png[width="800"]

[NOTE]
Due to the dependency of `Loan`s on `Bike`s, any bicycle that has been loaned out at least once will never be deleted from the system, merely archived and labelled `Out of Service`. The `Bike` can still be deleted completely if all of the `Loan`s associated with it are deleted (i.e. invalidated) first.
...
The current availability of bicycles are tracked by the system. Bicycles can thus be filtered by whether they are available or not, and the available ones are readily visible in the UI.
...
** Cons: Data on bikes have to be algorithmically searched for. Bicycles are assumed to exist and there is no verification for mistyped bicycle names.
