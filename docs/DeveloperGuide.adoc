...
Every `Loan` will have a reference to exactly one `Bike`: the bicycle that is loaned out. Conversely, every `Bike` has a reference to a list of `Loan`s, which tracks the bicycle's loan history and future reservations.
